#- input parameters: dataframe (`target`), dataframe (`features`), integer (`start_index`) and integer (`end_index`)
#- logics: subset the target and features based on the specifies indexes
#- output parameters: subsetted the target and features
...
#- input parameters: dataframe (`df`), column name of the target (`target_col`) and ratio for splitting (`test_ratio` default: 0.2)
...
#create a function called `pop_target` with the following logics:
...
#create a function called `split_sets_random` with the following logics:
